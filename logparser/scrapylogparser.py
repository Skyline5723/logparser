...
class ScrapyLogParser(Common):
    ...
    def main(self):
        ...
        self.data["last_update_time"], self.data["last_update_timestamp"] = (
            self.get_current_time_timestamp()
        )
        self.data["logparser_version"] = __version__
        return self.data

    def re_search_final_match(self, pattern, default="", step=-1, return_line=True):
    ...
    def extract_time(self):
        self.data["first_log_time"] = self.re_search_final_match(
            rf"\s\|\s({self.DATETIME_PATTERN})\s\|\s", step=1, return_line=False
        )
        self.data["latest_log_time"] = self.re_search_final_match(
            rf"\s\|\s({self.DATETIME_PATTERN})\s\|\s", return_line=False
        )

        if self.data["first_log_time"] and self.data["latest_log_time"]:
            first_log_datetime = self.string_to_datetime_obj(
                self.data["first_log_time"]
            )
            latest_log_datetime = self.string_to_datetime_obj(
                self.data["latest_log_time"]
            )
            self.data["runtime"] = str(latest_log_datetime - first_log_datetime)
            self.data["first_log_timestamp"] = self.datetime_obj_to_timestamp(
                first_log_datetime
            )
            self.data["latest_log_timestamp"] = self.datetime_obj_to_timestamp(
                latest_log_datetime
            )
        else:
    ...
    def extract_datas(self):
        ...
        self.data["datas"] = [
            [str(time_), int(pages), int(pages_min), int(items), int(items_min)]
    ...
    def extract_latest_matches(self):
        ...
        if "\\u" in latest_item:  # and sys.version_info.major < 3:
            try:
                self.data["latest_matches"]["latest_item"] = latest_item.encode(
                    "utf-8"
        ...
        for k in ["latest_crawl", "latest_scrape"]:
            ...
            if time_string:
                ...
                self.data["%s_timestamp" % k] = self.datetime_obj_to_timestamp(
                    datetime_obj
                )
            else:
    ...
    def extract_shutdown_reason(self):
        m = re.search(
            self.SIGTERM_PATTERN, self.re_search_final_match(self.SIGTERM_PATTERN)
        )
        self.data["shutdown_reason"] = m.group(1) if m else self.NA

    def extract_stats_dumped(self):
        self.data["finish_reason"] = (
            self.NA
        )  # May be updated in update_data_with_crawler_stats()
        ...
        if not (m and m.group(3)):
            ...
            self.update_data_with_crawler_stats(
                self.data, crawler_stats, update_log_count=True
            )
            self.data["crawler_stats"] = self.get_ordered_dict(
                crawler_stats, source="log"
            )
            time_string = m.group(1)
            datetime_obj = self.string_to_datetime_obj(time_string)
            self.data["crawler_stats"]["last_update_time"] = time_string
            self.data["crawler_stats"]["last_update_timestamp"] = (
                self.datetime_obj_to_timestamp(datetime_obj)
            )
